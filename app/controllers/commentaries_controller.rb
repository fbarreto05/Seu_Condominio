class CommentariesController < ApplicationController
  ...
  def update
    respond_to do |format|
      if @commentary.update(commentary_params)
        format.html { redirect_to @commentary, notice: "Commentary was successfully updated.", status: :see_other }
  ...
  private
    ...
    def set_commentary
      @commentary = Commentary.find(params.expect(:id))
    end

    # Only allow a list of trusted parameters through.
    def commentary_params
      params.expect(commentary: [ :name, :text, :task_id, :date ])
    end
end
